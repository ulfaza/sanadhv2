...
<html lang="en">
	<head>
		...
		<meta charset="utf-8" />
		<title> Edit Pembimbing - Paper</title>
				...
				<div class="main-content-inner">
					<div class="breadcrumbs ace-save-state" id="breadcrumbs">
						<ul class="breadcrumb">
							...
							<li>
								<a href="{{route('index.pembimbing')}}">Pembimbing</a>
							</li>
							<li>
								<a href="{{route('view.pembimbing', $tingkat)}}">Kelas {{ $tingkat }}</a>
							</li>
						</ul><!-- /.breadcrumb -->
					</div>
					...
					<div class="page-content">
						<div class="page-header">
							...
							<h4>
								Edit Pembimbing Paper <br> <br>
								{{ $row->s_nama }} <br>
								{{ $row->tingkat }} {{ $row->k_nama }} 
							</h4>
						</div><!-- /.page-header -->
						<div class="row">
							<div class="col-xs-12">
								<!-- PAGE CONTENT BEGINS -->
								@yield('content')
								<form action="{{route('update.pembimbing', [$tingkat, $row->p_id])}}" method="post" class="form-horizontal" role="form" >
									{{ csrf_field() }}
									<div class="form-group">
										<label class="control-label col-xs-12 col-sm-3 no-padding-right"> Pembimbing </label>

										<div class="col-xs-12 col-sm-9">
											<select  id="pembimbing" name="pembimbing" class="select2" >
												<option>{{ $row->pembimbing }}</option>
												@foreach($guru as $g)
